...
import { getPartRequestsByCustomerId, getCustomerById, tenantExists } from '@/lib/db';
...
export async function GET(
  ...
  try {
    ...
    const requests = await getPartRequestsByCustomerId(tenantId, customerId);
    return NextResponse.json({ requests });
  } catch (err: unknown) {
    console.error('Error fetching customer part requests:', err);
